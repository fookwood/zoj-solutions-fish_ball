// 1655632 2008-10-04 01:13:19 Accepted  2486 C++ 0 184 呆滞的慢板 
...
const BigNum power(const BigNum& a, int N) {
    if(N == 1) return a;
    BigNum p = power(a, (N/2));
    p = p * p;
    if(N&1) p = p * a;
    return p;
}
...
int main() {
    ...
    #define FUCK_THE_DATA
    #ifndef FUCK_THE_DATA
        // 为什么删掉下面这句就 WA 呢？狂不解！！ 
        // 测了老半天，测试数据问题啊 ！！真阴！！ 
        if(power(r, n) > p && power(r-1, n) < p)
             while(1) puts("fuck the data!!!");
    #else 
        if(power(r, n) > p) r = r - 1;
    #endif 
        cout << r << endl;
    }
}
