// 1683696 2008-10-30 23:23:27 Accepted  2403 C++ 0 184 呆滞的慢板 

// 一层一层往上算，每次相邻两个生成一个，因此每处理一层少一个，最后剩一个输出坐标。 
// 至于两个生成一个的方法，找出中点，加上一个垂直于圆心连线，长度为 sqrt(4 - (dist/2)^2) 的向量即可。 
...
int main() {
    ...
    while(cin >> N && N) {
        ...
        for(int n = N; n > 0; --n) {
            B.clear();
            for(int i = 1; i < n; ++i) {
                Point p = (A[i] + A[i-1]) / 2.0;
                Point v = ~(A[i] - A[i-1]);
                v = v / v.length();
                if(ng(v.y)) v = -v;
                double s = dist(A[i], A[i-1]) / 2.0;
                B.push_back(p + sqrt(4.0 - s * s) * v);
            }
            A = B;
        }
        printf("%.4lf %.4lf\n", A[0].x, A[0].y);
