// 1661084 2008-10-08 21:33:50 Accepted  2419 C++ 110 1932 呆滞的慢板  

// O(n) 的旋转卡壳，先凸包，然后选取开头三个点 p,q,r 开始旋转
// 注意 r 不超过第一个点，q 不超过 r，p 不超过 q 
// 每次做三次推进，先推进 r，使 pq 不动面积最大
// 然后推进 q，再推进 p，如果三次都没有推进过，r 推进一格
// 每次推进完一个点都更新一下面积最大值 
...
int main() {
    ...
    while(scanf("%d", &N) && N > 0) {
        ...
        while(r != 0) {
            ...
            while((r+1) % SZ &&
                ((CH[r]-CH[p])^(CH[q]-CH[p])) <
                ((CH[(r+1)%SZ]-CH[p])^(CH[q]-CH[p]))) {
                r = (r+1)%SZ;
                none = false;
            }
            ans = max(ans, area(CH[p], CH[q], CH[r]));
            while((q+1) % SZ != r &&
                ((CH[q]-CH[r])^(CH[p]-CH[r])) <
                ((CH[(q+1)%SZ]-CH[r])^(CH[p]-CH[r]))) {
                q = (q+1)%SZ;
                none = false;
            }
            ans = max(ans, area(CH[p], CH[q], CH[r]));
