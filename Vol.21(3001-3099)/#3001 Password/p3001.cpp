// 3057478 2008-09-02 16:38:50 Accepted 3001 C++ 00:01.50 936K 呆滞的慢板 

// 拉格朗日插值，公式见 C++数值算法 第二版，Section 3.1
// 蛮力套公式，加约分优化，加大数，这才乱搞过了，ft... 
...
pair<bool, BigNum> eval( int p ) {
    ...
    set_difference( A.begin(), A.end(),
        B.begin(), B.end(), back_inserter<vector<int> >( C ) );
    for( int i = 0; i < C.size(); ++i )
        ans *= C[i];
    C.clear();
    set_difference( B.begin(), B.end(),
        A.begin(), A.end(), back_inserter<vector<int> >( C ) );
    for( int i = 0; i < C.size(); ++i )
        ans /= C[i];
    return  make_pair(sign, ans);
}
...
int main() {
    ...
    for( cin >> T; T--; ) {
        ...
        for( int i = 0; i < N; ++i ) {
            pair<bool, BigNum>  pp = eval( i );
            if( pp.first )  ans -= pp.second;
            else            ans += pp.second;
        }
