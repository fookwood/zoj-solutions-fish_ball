// 1790511 2009-03-16 00:17:27 Accepted  3099 C++ 130 184 呆滞的慢板 

// 很巧妙的几何题，用一种特殊的贪心方法可以达到，详见解题报告。
...
int main() {
    ...
    for(scanf("%d", &T); T--;) {
        ...
        for(scanf("%d", &N); N--;) {
            ...
            bool xa = p.within(a), xb = p.within(b);
            if(xa && !xb)
                va.push_back(make_pair(p.area(), H));
            else if(!xa && xb)
                vb.push_back(make_pair(p.area(), H));
        }
        ...
        for(int i = 1; i < vb.size(); ++i) {
            if(vb[i - 1].second > vb[i].second)
                down += vb[i - 1].second - vb[i].second;
            else
                up += vb[i].second - vb[i - 1].second;
        }
        printf("%d %d\n", down, up);
    }
}
