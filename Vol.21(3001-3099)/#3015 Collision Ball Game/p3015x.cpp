...
const Point reflect( Line l, Point p ) {
    Line v( p, -1.0 / l.slope() );
    return intersect( v, l ) * 2.0 - p;
}
const Point reflect( Point p, Line l ) { return reflect( l, p ); }
double dist( Point p, Line l ) { return (l.A*p.x+l.B*p.y+l.C)/hypot(l.A,l.B); }
double dist( Line l, Point p ) { return (l.A*p.x+l.B*p.y+l.C)/hypot(l.A,l.B); }
...
int main() {
    ...
    double  k, h, a, b;
    while( cin >> k >> h >> a >> b ) {
        Line    L( Point( 0, h ), -tan( deg2rad( k ) ) );
        Point   A( 0, a ),  B( b, 0 );
        cout.setf( ios::fixed );
        cout.precision( 2 );
