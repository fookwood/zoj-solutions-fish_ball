// 1760662 2009-02-08 03:03:22 Accepted  1536 C++ 90 18116 呆滞的慢板 

// 带状态最短路，用一个变相的广搜累加即可，注意结果很大，记得用大数！ 
...
int main() {
    ...
    while(cin >> N >> K) {
        ...
        DQ.clear();
        DP[0][0][0] = 1;
        DQ.insert(make_pair(0, 0));
        while(!DQ.empty()) {
            int k = DQ.begin()->first;
            int x = DQ.begin()->second % 30;
            int y = DQ.begin()->second / 30;
            DQ.erase(DQ.begin());
            BigNum val = DP[x][y][k];
            for(int d = 0; d < 4; ++d) {
                x += dx[d];
                ...
                if(x >= 0 && x < N && y >= 0 && y < N && G[x][y] == '0') {
                    if(k + 1 <= K) {
                        DP[x][y][k + 1] = DP[x][y][k + 1] + val;
                        DQ.insert(make_pair(k + 1, y * 30 + x));
                    }
                }
                x -= dx[d];
                y -= dy[d];
