// 1662384 2008-10-10 09:29:30 Accepted  2102 C++ 0 184 呆滞的慢板 

// 把棍子等比分成若干段，然后在每个分段点看看是否落在任一个圆上 
// 看落在圆上的点中，第一个和最后一个之间是否包含了棍子中点。 
...
int main() {
    while(cin >> N && N) {
        ...
        memset(B, 0, sizeof(B));
        for(int t = 0; t < DIV; ++t) {
            p = ((p1 * (t+.5)) + (p2 * (DIV-t-.5))) / double(DIV);
            for(int i = 0; i < N; ++i) {
                if(le(dist(p, CC[i].c), CC[i].r)) {
                    B[t] = true;
                    break;
                }
            }
        ...
        int l = 0; while(!B[l]) l++;
        int r = DIV-1; while(!B[r]) r--;
        puts(l <= DIV/2 && r > DIV/2? "STAY" : "FALL");
    }
}
