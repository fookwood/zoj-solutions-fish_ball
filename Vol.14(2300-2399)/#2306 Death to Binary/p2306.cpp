// 1661614 2008-10-09 14:24:36 Accepted  2306 C++ 10 348 呆滞的慢板 

// fibonacci 进制数计算，只需模拟字符串。 
...
int main() {
    ...
    BigNum x;
    while(cin >> s1 >> s2) {
        s3 = froFib(toFib(s1)+toFib(s2));
        s1 = froFib(toFib(s1));
        s2 = froFib(toFib(s2));
        s1 = string(s3.size()-s1.size(), ' ') + s1;
        s2 = string(s3.size()-s2.size(), ' ') + s2;
        cout << "  " << s1 << endl;
        cout << "+ " << s2 << endl;
        cout << "  " << string(s3.size(), '-') << endl;
        cout << "  " << s3 << endl;
        puts("");
    }
}
