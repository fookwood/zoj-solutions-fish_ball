...
public:
    ...
    vector<Point> run( bool exclude = true ) {
        int i,  k = 0;
        bool ( *op ) ( double ) = exclude ? ng : np;
        for( refP = P[0], i = 1; i < P.size(); ++i ) {
        ...
        for( i = k = 3; i < P.size(); ++i, ++k ) {
            while( k > 2 && op( xmult( ans[k-2], P[i], ans[k-1] ) ) )
                { ans.pop_back(); k--; }
            ans.push_back( P[i] );
        }
        return  ans;
