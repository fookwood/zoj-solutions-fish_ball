...
int main() {
    ...
    for( int T = 0; scanf( "%d%d", &V, &E ) != EOF; ) {
        ...
        for( int i = 0; i < E; ++i ) {
            ...
            mf.insert( EE[i].v-1, EE[i].w-1, EE[i].c );
        }
        
        mf.runCut( 0, V-1 );
        
        vector<int> vV = mf.getCutV();
        vector<Edge*> eE = mf.getCutE();
        
        for( int i = 0; i < vV.size(); ++i )
            cout << vV[i] << ' ';puts("");
        for( int i = 0; i < eE.size(); ++i ) {
            cout << eE[i] - &mf.edges()[0] << ": ";
            cout << eE[i]->v<<' ' << eE[i]->w<<' '<<eE[i]->f<<'/'<<eE[i]->c<<endl;
        }
