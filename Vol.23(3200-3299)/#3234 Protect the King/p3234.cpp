// 2073182 2010-01-08 23:16:08 Accepted  3234 C++ 420 184 呆滞的慢板 

// 给一个点集，求不相交的多边形最多的层数。
// 实际就是求凸包的层数，一直凸包，剔除，直到点数小于三。 
...
int main() {
    ...
    while(scanf("%d", &N) == 1) {
        ...
        while(P.size() >= 3) {
            ...
            Polygon H = P;
            P.clear();
            Polygon G = ConvexHull(H);
            sort(H.begin(), H.end());
            sort(G.begin(), G.end());
            int a1 = 0, a2 = 0;
            while(a1 < G.size() && a2 < H.size()) {
                if(a1 < G.size() && H[a2] == G[a1]) {
                    ++a1; ++a2;
                }
                else {
                    P.push_back(H[a2++]);
