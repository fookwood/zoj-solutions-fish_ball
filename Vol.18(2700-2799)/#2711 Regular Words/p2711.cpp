// 1685566 2008-11-01 19:40:35 Accepted  2711 C++ 930 184 呆滞的慢板 

// 经典 DP + 滚动数组，DP[K][A][B][C] 表示现有 K 个字符时 A,B,C 的个数。 
...
bool operator <(const Idx& a, const Idx& b) {
    return ((a.A*61)+a.B)*61+a.C < ((b.A*61)+b.B)*61+b.C;
}

map<Idx, BigNum> X, Y;
map<Idx, BigNum>::iterator it;
    ...
    while(cin >> N) {
        ...
        for(int k = 0; k < N3; ++k) {
            ...
            for(it = X.begin(); it != X.end(); ++it) {
                ...
                C = it->first.C;
                if(A+1 <= N) {
                    BigNum& p = Y[Idx(A+1, B, C)];
                    p = p + it->second;
                }
                if(B+1 <= A) {
                    BigNum& p = Y[Idx(A, B+1, C)];
                    p = p + it->second;
                }
                if(C+1 <= B) {
                    BigNum& p = Y[Idx(A, B, C+1)];
                    p = p + it->second;
                }
            }
            swap(X, Y);
        }
        cout << X[Idx(N, N, N)] << endl;
        puts("");
    }
}
