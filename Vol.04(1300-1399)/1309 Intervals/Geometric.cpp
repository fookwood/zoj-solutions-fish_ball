...
//    freopen( "testout.txt", "w", stdout );
    ...
    while( cin >> N && N ) {
        cin >> p;
        Q.assign( 1, make_pair( INF, INF ) );
        while( N-- ) {
            cin >> c.c >> c.r;
            Assert2( lt( c.c.y+c.r, p.y ) );
            pp = tangent( p, c );
            double  low = intersect( Line( Point(), 0.0 ), pp.second ).x,
                    high = intersect( Line( Point(), 0.0 ), pp.first ).x;
            if( low > high )    swap( low, high );
            while( Q.front().first != INF ) {
                double  plow = Q.front().first,
                        phigh = Q.front().second;
                Q.pop_front();
                if( lt( phigh, low ) || gt( plow, high ) )
                    Q.push_back( make_pair( plow, phigh ) );
                else {
                    low = min( low, plow );
                    high = max( high, phigh );
                }
...
//            Q[i].first = round( Q[i].first * 100.0 ) / 100.0 + EPS;
//            Q[i].second = round( Q[i].second * 100.0 ) / 100.0 + EPS;
            cout << Q[i].first - EPS << ' ' << Q[i].second + EPS << endl;
        }
        puts( "" );
    }
