// 1796724 2009-03-21 15:40:45 Accepted  2061 C++ 70 8112 呆滞的慢板 

// 买票，经典组合数学 DP，先用类似于求组合数的手法 DP 出不区分的种类数。
// 然后将结果乘以 M! * N! 即可得到全排列数，记得用大数。 
    ...
    while(cin >> M >> N && (M || N)) {
        ...
        for(int i = 0; i < M + N; ++i) {
            for(int j = 0; j <= i && j <= M; ++j) {
                int x = j, y = i - j;
                if(j < M) A[i + 1][j + 1] = A[i + 1][j + 1] + A[i][j];
                if(i - j < N && i + 1 - j <= j) A[i + 1][j] = A[i + 1][j] + A[i][j];
            }
        }
        BigNum ans = A[M + N][M];
