// 1687402 2008-11-03 11:55:40 Accepted  2896 C++ 10 184 呆滞的慢板 

// 求两个多项式是否有公因式：
// 多项式除和求余用解卷积， 
// 用辗转相除法求最大公因式，判断其阶是否大于一即可。 
...
int main() {
    ...
    while(cin >> M >> N) {
        m.resize(M+1);
        n.resize(N+1);
        for(int i = M; i >= 0; --i) cin >> m[i];
        for(int i = N; i >= 0; --i) cin >> n[i];
        gcd(m, n, r);
