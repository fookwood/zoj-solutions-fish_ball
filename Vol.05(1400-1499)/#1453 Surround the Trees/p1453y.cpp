// 3051851 2008-08-26 23:12:10 Accepted 1453 C++ 00:00.08 2992K 呆滞的慢板 
...
class ConvexHull {
    ...
    static bool acmp( const Point& a, const Point& b )
        { return  ( a - refP ) * ( b - refP ) < 0.0; }
...
public:
    ...
    void insert( const Point& p ) { V.push_back( p ); }
    ...
    vector<Point> run() {
        ...
        swap( V[0], V[pos] );
        refP = V[0];
        sort( V.begin() + 1, V.end(), acmp );
//for(int i=0;i<V.size();++i)cout<<V[i]<<endl;     puts("");   

        if( V.size() < 3 ) return  V;
        vector<Point>   ans( V.begin(), V.begin() + 2 );
