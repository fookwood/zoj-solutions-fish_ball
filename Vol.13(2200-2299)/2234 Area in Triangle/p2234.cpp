...
//  V2.1(2009-3-21):                                                    //
//      -Functions to handle more relation between objects added:       //
//          *Method: perpendicular(Point, Line)                         //
//          *Method: angelBisector(Point, Point, Point)                 //
//      -More triangle functions added:                                 //
//          *Method: angel_A(double, double, double)                    //
//          *Method: outcircleRadius(double a, double b, double c)      //
//          *Method: incircleRadius(double a, double b, double c)       //
...
double outcircleRadius(double a, double b, double c)
    { return a * b * c / 4 / areaSSS(a, b, c); }
double incircleRadius(double a, double b, double c) 
    { return areaSSS(a, b, c) * 2 / (a + b + c); }
double angle_A(double a, double b, double c)
    { return acos((b*b + c*c  - a*a) * 0.5 / b / c); }
...
int main() {
    ...
    double a, b, c, l, d, A, B, C, fA, fB, fC, fD, fE, fF;
    int T = 1;

    while(cin >> a >> b >> c >> l && (a || b || c || l)) {
        d = incircleRadius(a, b, c);
        A = angle_A(a, b, c);
        B = angle_A(b, a, c);
        C = angle_A(c, a, b);
        double ans;
        if(ge(l, a + b + c)) {
            ...
            double kx = tan(A / 2) - 0.5 * (PI - A) * tan(A / 2);
            double ky = tan(B / 2) - 0.5 * (PI - B) * tan(B / 2);
            double kz = tan(C / 2) - 0.5 * (PI - C) * tan(C / 2);
            double de = 2 + tan(C / 2) * (PI - C);
            double pa = (a + b + c) / de;
            double vx = -(2 + tan(A / 2) * (PI - A)) / de;
            double vy = -(2 + tan(B / 2) * (PI - B)) / de;
            fA = kx + kz * vx * vx;
            fB = 2 * kz * vx * vy;
            fC = ky + kz * vy * vy;
            fD = 2 * kz * pa * vx;
            fE = 2 * kz * pa * vy;
            fF = kz * pa * pa;
            double ax = (fB*fE - 2*fC*fD) / (4*fA*fC - fB*fB);
            double ay = (fB*fD - 2*fA*fE) / (4*fA*fC - fB*fB);
            ans = fA*ax*ax + fB*ax*ay + fC*ay*ay + fD*ax + fE*ay + fF;
            cout << ax << ' ' << ay << endl;
        }
        printf("Case %d: %.2lf\n", T++, ans);
    }
