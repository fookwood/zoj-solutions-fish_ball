...
double eval(double r) {
    if(r < dist(O, Line(B, C))) return 2.0 * PI * r;
    double width = sqrt(r * r - d * d);
    double ans = 0;
    if(dist(MA, C) < width) ans += dist(MA, C) * 2;
    else {
        ans += 2 * width;
        Point lhs = MA + (C - MA) / (C - MA).length() * width;
        Point rhs = MB + (C - MB) / (C - MB).length() * width;
        ans += asin(dist(lhs, rhs) / 2 / r) * 2 * r;
    }
    if(dist(MA, B) < width) ans += dist(MA, B) * 2;
    else {
        ans += 2 * width;
        Point lhs = MA + (B - MA) / (B - MA).length() * width;
        Point rhs = MC + (B - MC) / (B - MC).length() * width;
        ans += asin(dist(lhs, rhs) / 2 / r) * 2 * r;
    }
    if(dist(MC, A) < width) ans += dist(MC, A) * 2;
    else {
        ans += 2 * width;
        Point lhs = MC + (A - MC) / (A - MC).length() * width;
        Point rhs = MB + (A - MB) / (A - MB).length() * width;
        ans += asin(dist(lhs, rhs) / 2 / r) * 2 * r;
    }
    return ans;
}
...
double getArea(double r) {
    if(r < dist(O, Line(B, C))) return PI * r * r;
    double width = sqrt(r * r - d * d);
    double ans = 0;
    if(dist(MA, C) < width) ans += dist(MA, C) * d;
    else {
        ans += d * width;
        Point lhs = MA + (C - MA) / (C - MA).length() * width;
        Point rhs = MB + (C - MB) / (C - MB).length() * width;
        ans += asin(dist(lhs, rhs) / 2 / r) * r * r;
    }
    if(dist(MA, B) < width) ans += dist(MA, B) * d;
    else {
        ans += d * width;
        Point lhs = MA + (B - MA) / (B - MA).length() * width;
        Point rhs = MC + (B - MC) / (B - MC).length() * width;
    ...
    if(dist(MC, A) < width) ans += dist(MC, A) * d;
    else {
        ans += d * width;
        Point lhs = MC + (A - MC) / (A - MC).length() * width;
        Point rhs = MB + (A - MB) / (A - MB).length() * width;
...
cout << O << endl;
        ...
        MC = pedal(O, Line(B, A));
puts("##########");
cout << dist(O, Line(A, B))<< endl;
cout << dist(O, Line(A, C))<< endl;
cout << dist(O, Line(C, B))<< endl;
cout << dist(O, MA)<< endl;
cout << dist(O, MB)<< endl;
cout << dist(O, MC)<< endl;
puts("##########");
        d = dist(O, Line(A, B));
        double left = l / 2.0 / PI;
        double right = max(max(dist(O, A), dist(O, B)), dist(O, C));
cout << "min = " << left << endl;
cout << "max = " << right << endl;
double step = (right - left) / 100;
for(double rr = left; rr <= right + EPS; rr += step)
cout << rr << ": " << eval(rr) << endl;
        while(right - left > EPS) {
            double middle = (right + left) / 2;
            if(eval(middle) < l) left = middle;
            else right = middle;
        }
cout << right << endl;
        printf("Case %d: %.2lf\n", T++, getArea(right));
